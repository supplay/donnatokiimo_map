...
移動式焼き芋屋の現在地をリアルタイムで確認できるアプリです。

### 店主画面
https://github.com/user-attachments/assets/ec983d2e-8710-48e5-94f6-8b544e5885c4


### ユーザー画面
https://github.com/user-attachments/assets/95f543bd-6de9-482c-8274-a41c28b76864
...
## 🚧 今後の改善
* 予約機能の追加  
  アプリから既存の公式LINEへ遷移し、Amazon Lexのチャットボットで予約受付を行う構想です。  
  取得した予約情報（日時・名前・数量・場所）はAmazon SQSを介して非同期にDynamoDBへ保存し、店主側で一覧表示できるようにする予定です。  
  また、処理に失敗したメッセージについてはデッドレターキュー（DLQ）に退避させることで、予約の取りこぼしを防ぎ、後から再処理できる仕組みを検討しています。  
  これにより、アクセス集中時の安定性や障害時の復旧性を高めることを想定しています。  
  一方で、チャットボットの精度次第では確認作業が増え、店主の負担になる可能性もあるため、業務効率化につながるかを見極めながら慎重に実装していきます。
* 通知精度のチューニング
* UI/UX改善
